# Spec: <feature name>

> Context: see this repo's [../CLAUDE.md](../CLAUDE.md) for architecture and conventions.
> Don't re-explain the system — describe only what changes.
> Cross-repo feature? Link the companion specs in the other repos (see [README.md](./README.md)).

## Goal

<One or two sentences: what should be true after this is built, and for whom.>

## Background / motivation

<Why now? The problem this solves.>

## Behavior & acceptance criteria

<Concrete, checkable statements. Prefer observable behavior over implementation.>

- [ ]
- [ ]
- [ ]

## Changes in this repo

<Modules / endpoints / components / workflows that change here.>

## Cross-repo coordination

<If this feature spans repos: name the other affected repos and link their companion
specs. Mark one repo's spec as the "lead". Write "Self-contained" if it touches only this repo.>

## Data / schema changes

<New tables/columns/migrations/config. "None" is valid.>

## Out of scope

<What this spec explicitly does NOT cover.>

## Open questions

<Anything undecided that needs an answer before or during implementation. Once a question is
answered, remove it from here and fold the decision into the acceptance criteria / changes —
this section should only ever list what's still open.>
