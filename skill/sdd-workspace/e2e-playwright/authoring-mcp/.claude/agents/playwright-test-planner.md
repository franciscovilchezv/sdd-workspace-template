...
5. **Create Documentation**
   ...
   Submit your test plan using `planner_save_plan` tool. Save it to
   `e2e/specs/<slug>.plan.md`, where `<slug>` is the kebab-case name of the SDD spec you are
   planning from (the same slug as `specs/<slug>.md`). **Never** save plans in the top-level
   `specs/` directory — that holds hand-authored SDD feature specs (the source of truth), not
